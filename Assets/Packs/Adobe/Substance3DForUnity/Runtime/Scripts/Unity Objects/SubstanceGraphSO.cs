        ...
        public void UpdateOutputTextures(IntPtr renderResultPtr)
        {
            unsafe
            {
                foreach (var output in Output)
                {
                    ...
                    if (texture.format != srcImage.image_format.ToUnityFormat())
                    {
                        Debug.LogError($"Fail to update substance output: target Unity texture is not {srcImage.image_format.ToUnityFormat()}.");
                        continue;
                    }

                    texture.LoadRawTextureData(srcImage.data, srcImage.GetSizeWithMipMaps());
                    texture.Apply();
                }
            }
        }
